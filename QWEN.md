...
2. Стабы обновляются автоматически через `scripts/gen_stubs.py` (pre-commit hook)
3. Добавить Python пример в `examples/`
4. Добавить smoke тест в `tests/python/`

**Важно:** `.pyi` файлы генерируются автоматически из макросов. Не редактируйте их вручную — CI проверяет актуальность стабов.
...
- Проверка актуальности стабов: `python scripts/gen_stubs.py && git diff --exit-code`
