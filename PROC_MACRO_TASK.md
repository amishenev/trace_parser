...
| `#[field(ty, name, optional, readonly, private)]` | Атрибуты полей |

### Атрибуты полей

| Атрибут | Значение | Python API | Пример |
|---------|----------|------------|--------|
| `ty` | Тип парсинга | — | `#[field(ty = "u32")]` |
| `name` | Имя в payload | — | `#[field(name = "prev_comm")]` |
| `optional` | Опциональное поле | — | `#[field(optional)]` |
| `readonly` | Только чтение | `#[pyo3(get)]` | `#[field(readonly)]` |
| `private` | Приватное | Не экспортируется | `#[field(private)]` |

### Поддерживаемые типы полей (`ty`)
...
- [x] Регистрация через `register_tracing_mark_parser!`
...
|------|--------|-------------|-----------------|
| 1. Настройка | ✅ Завершено | 2026-03-31 | 2026-03-31 |
...
| 5. Интеграция | ✅ Завершено | 2026-03-31 | 2026-03-31 |
| 6. Тесты и документация | ✅ Завершено | 2026-04-02 | 2026-04-02 |

**Общий прогресс:** 6/6 этапов (100%)
...
1. **`render_payload`** — генерация кода рендера полей на основе field specs
   - Поддержка всех типов: string, u32, i32, f64, bool_int
   - Поддержка optional полей
   - Поддержка кастомных имён полей

2. **`parse_payload`** — генерация кода парсинга из regex captures
   - Поддержка всех типов полей
   - Optional поля возвращают `Option<T>`
   - Required поля используют `?` оператор

3. **`field_accessors`** — геттеры/сеттеры для Python API
   - Required поля: getter `&T`, setter `T`
   - Optional поля: getter/setter `Option<T>`
   - Readonly поля: только getter
   - Private поля: исключаются из Python API

4. **`detect_format`** — детекция формата для нескольких темплейтов
   - Явный `id` через `#[define_template("...", id = 0)]`
   - Auto-assign: `id = max(previous_ids) + 1`
   - Примеры:
     - `[None, None]` → `[0, 1]`
...
5. **Тесты**
   - 41 unit тест в `macros/src/generator.rs`
   - Параметризированные тесты через `rstest`
   - Интеграционные тесты генерации кода

### ⏳ Отложено

- **E2E тесты с реальным событием** — сложности с интеграцией макроса в тестовый модуль
- Требуется дополнительная проработка архитектуры

---

## Статистика

- **Тестов:** 41 (macros) + 41 (основной crate)
- **Код макроса:** ~650 строк
- **Документация:** `macros/QWEN.md`, `macros/AGENTS.md`
