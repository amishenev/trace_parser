...
def main() -> None:
    ...
    exited2 = TraceExit.parse(lines[3])
    if exited2 is not None:
        print("exit2", exited2.pid, exited2.comm, exited2.exit_tgid)

    devfreq = TraceDevFrequency.parse(lines[4])
    if devfreq is not None:
        print("devfreq", devfreq.clk, devfreq.state)

    vsync = TraceReceiveVsync.parse(lines[5])
    if vsync is not None:
        print("vsync", vsync.frame_number, vsync.begin.trace_mark_tgid)

    vsync_from_module = TraceReceiveVsyncModule.parse(lines[5])
